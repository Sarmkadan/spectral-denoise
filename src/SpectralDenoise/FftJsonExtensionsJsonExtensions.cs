...
namespace SpectralDenoise
{
    ...
    /// </summary>
    public static class FftJsonExtensionsJsonExtensions
    {
        ...
        private static Complex[] SanitizeNaNInfinity(Complex[] array)
        {
            ...
            for (int i = 0; i < array.Length; i++)
            {
                ...
                real = double.IsNaN(real) || double.IsInfinity(real) ? 0 : real;
                imaginary = double.IsNaN(imaginary) || double.IsInfinity(imaginary) ? 0 : imaginary;
        ...
        private static void EnsureValidComplexArray(this Complex[] array)
        {
            if (array == null)
            {
            ...
            for (int i = 0; i < array.Length; i++)
            {
                if (double.IsNaN(array[i].Real) || double.IsInfinity(array[i].Real))
                {
                    throw new ArgumentException(
                        $"Complex array element at index {i} has invalid real component (got {array[i].Real}).",
                        nameof(array));
                }
                ...
                {
                    throw new ArgumentException(
                        $"Complex array element at index {i} has invalid imaginary component (got {array[i].Imaginary}).",
                        nameof(array));
                }
            }
        }
