...
return Denoise(inputPath, outputPath, noiseSeconds, frameSize, hop);
...
static int Denoise(string inPath, string outPath, double noiseSeconds, int frameSize, int hop)
{
	// Try to read as stereo first
	try
	{
		var (left, right, sr) = WavFile.ReadStereo(inPath);
		...
		var profile = sub.EstimateNoiseProfile(left.AsSpan(0, noiseLen));
